...
import (
	...
	airline "github.com/uowcs/CSIT214/internal/airline"
...
func buildServer(env config.EnvVars) (*fiber.App, func(), error) {
	// init the storage
	db, err := storage.BootstrapPostgres(env.DATABASE_URL)
	if err != nil {
		return nil, nil, err
	}
	...
	airlinestore := airline.NewAirlineStorage(db)
	airlinecontroller := airline.NewAirlineController(airlinestore)
	airline.AddAirlineRoutes(app, airlinecontroller)
